...
KEY_SCHEDULE_NAME           = 'name'            # 定时名称
KEY_SCHEDULE_REPEAT_TYPE    = 'repeat_type'     # 定时重复类型
KEY_SCHEDULE_REPEAT_VALUE   = 'repeat_value'    # 定时的重复周期
KEY_SCHEDULE_HOUR           = 'hour'            # 定时执行的小时
KEY_SCHEDULE_MINUTE         = 'minute'          # 定时执行的分钟
KEY_SCHEDULE_TARGET         = 'target'          # 定时控制的目标
KEY_SCHEDULE_VALUE          = 'value'           # 定时控制的动作
KEY_SCHEDULE_IS_ACTIVE      = 'is_active'       # 定时是否打开
KEY_SCHEDULE_START_TIME      = 'start_time'       # 定时是否打开
KEY_SCHEDULE_PREV_TIME      = 'prev_time'       # 上次执行的时间
KEY_SCHEDULES               = 'schedules'       # 定时的具体内容
...
# 设置定时(添加或者编辑)
def add_schedule(scheduleObj):
    config = configparser.ConfigParser()
    config.read(FILE_NAME_CONFIG)
    scheduleDict = get_schedule()
    print('原来的内容为 : '+str(scheduleDict)+'  类型为'+str(type(scheduleDict)))
    # if len(scheduleDict) > 0:
    #     for tempSchdule in scheduleDict :
    #         if tempSchdule['name'] == scheduleObj.name:
    #             return
    # if len(scheduleDict) > 0:
    #     for tempSchdule in scheduleDict:
    #         print('其中的一个定时 => '+str(tempSchdule))
    scheduleDict['data'].append(scheduleObj.toDict())
    print('要写入的内容为 : '+str(scheduleDict))
    __write_value(config,SECTIONS_SCHEDULE,KEY_SCHEDULES,str(scheduleDict).replace("'",'"'))
    config.write(open(FILE_NAME_CONFIG,'w'))
...
def del_schedule(scheduleName):
    ...
    config.read(FILE_NAME_CONFIG)
    scheduleDict = get_schedule()
    scheduleList = scheduleDict.get('data',[])
    for tempSchdule in scheduleList:
        if tempSchdule.get('name',' ') == scheduleName:
            scheduleList.remove(tempSchdule)
    __write_value(config,SECTIONS_SCHEDULE,KEY_SCHEDULES,str(scheduleDict).replace("'",'"'))
    config.write(open(FILE_NAME_CONFIG,'w'))

def edit_schedule(scheduleObj,oldScheduleName):
    config = configparser.ConfigParser()
    config.read(FILE_NAME_CONFIG)
    scheduleDict = get_schedule()
    scheduleList = scheduleDict.get('data',[])
    for tempSchdule in scheduleList:
        if tempSchdule.get('name',' ') == oldScheduleName :
            tempSchdule['name'] = scheduleObj.name
            tempSchdule['repeat_type'] = scheduleObj.repeat_type
            tempSchdule['repeat_value'] = scheduleObj.repeat_circle
            tempSchdule['hour'] = scheduleObj.hour
            tempSchdule['minute'] = scheduleObj.minute
            tempSchdule['target'] = scheduleObj.target
            tempSchdule['value'] = scheduleObj.value
            tempSchdule['is_active'] = scheduleObj.is_active
    __write_value(config,SECTIONS_SCHEDULE,KEY_SCHEDULES,str(scheduleDict).replace("'",'"'))
    config.write(open(FILE_NAME_CONFIG,'w'))

def edit_schedule_inner(scheduleObj,oldScheduleName):
    config = configparser.ConfigParser()
    config.read(FILE_NAME_CONFIG)
    scheduleDict = get_schedule()
    scheduleList = scheduleDict.get('data',[])
    for tempSchdule in scheduleList:
        if tempSchdule.get('name',' ') == oldScheduleName :
            tempSchdule['name'] = scheduleObj['name']
            tempSchdule['repeat_type'] = scheduleObj['repeat_type']
            tempSchdule['repeat_value'] = scheduleObj['repeat_value']
            tempSchdule['hour'] = scheduleObj['hour']
            tempSchdule['minute'] = scheduleObj['minute']
            tempSchdule['target'] = scheduleObj['target']
            tempSchdule['value'] = scheduleObj['value']
            tempSchdule['is_active'] = scheduleObj['is_active']
            tempSchdule['prev_time'] = scheduleObj['prev_time']
    __write_value(config,SECTIONS_SCHEDULE,KEY_SCHEDULES,str(scheduleDict).replace("'",'"'))
    config.write(open(FILE_NAME_CONFIG,'w'))
...
def get_schedule():
    config = configparser.ConfigParser()
    config.read(FILE_NAME_CONFIG)
    return json.loads(__read_value(config,SECTIONS_SCHEDULE,KEY_SCHEDULES,'{"data":[]}'))
...
class Schedule:
    ...
    def toDict(self):
        return  {
                    KEY_SCHEDULE_NAME:self.name,
                    KEY_SCHEDULE_REPEAT_TYPE:self.repeat_type,
                    KEY_SCHEDULE_REPEAT_VALUE:self.repeat_circle,
                    KEY_SCHEDULE_HOUR:self.hour,
                    KEY_SCHEDULE_MINUTE:self.minute,
                    KEY_SCHEDULE_TARGET:self.target,
                    KEY_SCHEDULE_VALUE:self.value,
                    KEY_SCHEDULE_IS_ACTIVE:self.is_active,
                    KEY_SCHEDULE_START_TIME:self.start_time
                }
